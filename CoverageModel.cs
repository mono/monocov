...
namespace MonoCov
{
    ...
    public class CoverageModel : CoverageItem {
        ...
        private void parseCoverage(MethodCoverageItem method, LineNumberEntry[] lines, string cov_info, int[] coverage)
        {
    	...
    	private void computeNewMethodCoverage (MethodCoverageItem method, LineNumberEntry[] lines, string cov_info)
    	{
    		...
    		if (cov_info == null) {
                ...
                parseCoverage(method, lines, cov_info, coverage);
                /*
    			// Hand crafted parsing code since this is performance critical
    			int pos = 0;
    			int prev_offset = 0;
        ...
        private void updateMethodCoverage(MethodCoverageItem method, LineNumberEntry[] lines, string cov_info)
        {
            parseCoverage(method, lines, cov_info, method.lineCoverage);
            
            int hit = 0;
            int missed = 0;
    
            for (int i = 0; i < method.lineCoverage.Length; ++i) {
                int count = method.lineCoverage [i];
                if (count > 0)
                    hit ++;
                else if (count == 0)
                    missed ++;
    ...
    #endif
    	{
            ...
            if(!klass.methodsByMethod.ContainsKey(monoMethod))
            {
        		...
        		method = new MethodCoverageItem (klass, methodName);
        		...
        		computeNewMethodCoverage (method, lines, cov_info);
            }
            else
            {
                method = klass.methodsByMethod[monoMethod] as MethodCoverageItem;
                updateMethodCoverage(method, lines, cov_info);
            }
    	}
    }
}
